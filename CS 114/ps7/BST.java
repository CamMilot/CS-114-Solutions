...
class BST<K extends Comparable<K>, E> implements Dictionary<K, E> {
  ...
  protected void levelorderElements(BSTNode<K, E> v, ArrayList<E> elts) {
    ...
    while (!dq.isEmpty()) {
      ...
      for (int x = 0; x < size; x++) {
        BSTNode<K, E> current = dq.removeFirst();
  ...
  private List<BSTNode<K, E>> findhelpSeen(BSTNode<K, E> rt, K k, List<BSTNode<K, E>> Seen) {
    Seen.add(rt);
    if (rt == null)
      return null;
    if (rt.key().compareTo(k) > 0)
      return findhelpSeen(rt.left(), k, Seen);
    else if (rt.key().compareTo(k) == 0) {
      return Seen;
    } else
      return findhelpSeen(rt.right(), k, Seen);
  }

  private List<E> findhelpSill(BSTNode<K, E> rt, K k, List<BSTNode<K, E>> Seen,
      List<E> Between) {
    // for (BSTNode<K, E> curr : Seen) {
    // if(curr.equals(rt)){
    // return null;
    // }
    // }
    ...
    if (rt.key().compareTo(k) > 0) {
      Between.add(rt.left().element());
      return findhelpSill(rt.left(), k, Seen, Between);
    } else if (rt.key().compareTo(k) == 0) {
      Seen.add(rt);
      return Between;
    } else {
      // Between.add(rt.right().element());
      return findhelpSill(rt.right(), k, Seen, Between);
    }
  ...
  protected void inorderElements(BSTNode<K, E> v, ArrayList<E> elts) {
    ...
    if (v.right() != null)
      inorderElements(v.right(), elts);
    elts.add(v.element());// recurse on right child
  }
